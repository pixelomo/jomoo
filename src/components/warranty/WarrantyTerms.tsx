...
const EXCLUSIONS: { marker: string; text: ReactNode }[] = [
  ...
  {
    marker: '2）',
    text: '本来の用途以外での使用（車両、船舶、使用頻度が極めて高い商業用途など）に起因する故障および損傷、または無許可の修理（JOMOO認定サービス担当者以外による分解修理、純正部品以外を使用した修理）による損傷。',
  },
  {
    marker: '3）',
    text: '人為的・偶発的な損傷：砂、汚れた手、輸送などによる機械的損傷（傷、ひび割れなど）、または異物の内部混入による損傷。',
  },
  {
    marker: '4）',
    text: '自然災害・環境条件・公共の事象など：地震、火災、水害、落雷、その他の天災、公害、異常電圧、大気汚染、ガス汚染、小動物の行為および環境条件（凍害、カビ、異常電圧、異常水圧、動物・昆虫など）、戦争、暴動、犯罪などの不法行為に起因する故障および損傷。',
  },
  {
    marker: '5）',
    text: (
  ...
  {
    marker: '6）',
    text: '配管への異物（泥・砂など）の流入、水垢や配管の詰まりに起因する故障。',
  },
  {
    marker: '7）',
    text: '製品部品の経年使用による外観上の変化（変色、もらい錆など）や使用に伴う自然摩耗、および消耗品（電池、フィルター、発泡剤、パッキンの劣化など）。',
  },
  {
    marker: '8）',
    text: '保証書の有効期限切れ、保証書を提示できない場合、または本保証書が正しく記入されていない場合、記載内容が無断で変更された場合。',
  },
        ...
        <div className="warranty-section__inner warranty-terms">
          ...
          <ol>
            ...
            <li>
              ...
              <ol className="warranty-terms__sub--gap">
                {EXCLUSIONS.map(({ marker, text }) => (
                  <li key={marker}>
                    <span className="warranty-terms__marker">{marker}</span>
                    {text}
                  </li>
                ))}
              </ol>
