  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontFamily: 'ui-monospace,monospace', fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
...
export default async function ProductDetailPage({ params }: { params: Params }) {
  ...
  const isJa = locale === 'zh-CN'
  const name        = isJa ? product.name.zhCN        : product.name.en
  const tagline     = isJa ? product.tagline?.zhCN    : product.tagline?.en
  const description = isJa ? product.longDescription?.zhCN : product.longDescription?.en
  ...
  // Build carousel slides from Sanity images
  ...
  return (
    <main>
      ...
      >
        ...
        <span>/</span>
        <Link href="/products/smart-toilet" style={{ color: 'var(--text-muted)' }}>
          {t('smartToiletName')}
        </Link>
        <span>/</span>
      ...
      <div className="section-wrap jm-detail-grid" style={{ paddingTop: '40px' }}>
        ...
        <div>
          ...
          {/* Tagline */}
          {tagline && (
            <p style={{ fontSize: '16px', color: 'var(--text-soft)', lineHeight: 1.7, margin: '0 0 32px', letterSpacing: '0.02em' }}>
              {tagline}
            </p>
          )}
          ...
          {product.features && product.features.length > 0 && (
            <div style={{ marginBottom: '36px' }}>
              <h2 style={{ fontSize: '13px', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--text-muted)', margin: '0 0 16px' }}>
                Features
              </h2>
              <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {product.features.map((f, i) => (
                  <li key={i} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                    ...
                    <div>
                      <p style={{ fontWeight: 600, fontSize: '14px', margin: '0 0 2px', color: 'var(--foreground)' }}>
                        {isJa ? f.title.zhCN : f.title.en}
                      </p>
                      <p style={{ fontSize: '13px', color: 'var(--text-soft)', margin: 0, lineHeight: 1.6 }}>
                        {isJa ? f.description.zhCN : f.description.en}
                      </p>
                    </div>
                  </li>
                ))}
      ...
      {description && (
        <section style={{ background: '#f6f4ef', padding: '80px 0' }}>
          <div className="section-wrap" style={{ maxWidth: '760px', paddingTop: 0, paddingBottom: 0 }}>
            <h2 style={{ fontSize: '26px', fontWeight: 700, margin: '0 0 24px', letterSpacing: '0.02em' }}>
              製品について
            </h2>
      ...
      {product.featureVideos && product.featureVideos.length > 0 && (
        <section className="section-wrap" style={{ paddingBottom: 0 }}>
          <h2 style={{ fontSize: '26px', fontWeight: 700, margin: '0 0 8px', letterSpacing: '0.02em' }}>
            Feature Videos
          </h2>
          ...
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: product.featureVideos.length > 1 ? '1fr 1fr' : '1fr',
              gap: '32px',
            }}
          >
            {product.featureVideos.map((v, i) => (
              <div key={i}>
                <VideoPlaceholder
                  title={isJa ? v.title.zhCN : v.title.en}
                  embedUrl={v.embedUrl}
                />
                <p style={{ marginTop: '12px', fontSize: '14px', fontWeight: 500, color: 'var(--foreground)' }}>
                  {isJa ? v.title.zhCN : v.title.en}
                </p>
              </div>
            ))}
          </div>
          ...
          <tbody>
            ...
            {product.specTable && Object.entries(product.specTable).map(([key, val]) => {
              ...
              return (
                <SpecRow
                  key={key}
                  label={isJa ? label.zhCN : label.en}
                  value={val}
                />
              )
            })}
            ...
            {product.specs?.map((spec, i) => (
              <SpecRow
                key={`custom-${i}`}
                label={isJa ? spec.label.zhCN : spec.label.en}
                value={spec.value}
              />
            ))}
          </tbody>
