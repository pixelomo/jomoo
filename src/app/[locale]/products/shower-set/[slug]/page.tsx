...
export default async function ShowerSetDetailPage({ params }: { params: Params }) {
  ...
  const specLabels: Record<string, { zhCN: string; en: string }> = {
    dimensions:       { zhCN: 'サイズ',            en: 'Dimensions'             },
    material:         { zhCN: '素材',              en: 'Material'               },
    power:            { zhCN: '流量',              en: 'Flow Rate'              },
    drainageMethod:   { zhCN: '温度調節範囲',      en: 'Temperature Range'      },
    waterConsumption: { zhCN: 'シャワーヘッド径',  en: 'Shower Head Diameter'   },
  ...
  return (
    <main>
      <div className="section-wrap" style={{ paddingTop: '24px', paddingBottom: 0, fontSize: '13px', color: 'var(--text-muted)', display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <span>/</span>
        <Link href="/products/shower-set" style={{ color: 'var(--text-muted)' }}>{t('showerSetName')}</Link>
        <span>/</span>
