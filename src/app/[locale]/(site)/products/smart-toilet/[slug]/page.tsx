...
export default async function ProductDetailPage({ params }: { params: Params }) {
  ...
  // Exclude GIFs from the carousel (no animation in Sanity CDN WebP pipeline)
  const featureImgs = (product.featureImages ?? []).filter(fi => !fi.asset?._ref?.endsWith('-gif'))

  // Carousel: hero image + non-GIF feature images
  const slides = [
    ...(product.images ?? []).map(img => ({
      src:   img.asset ? imgUrl(img.asset, 1200) : undefined,
      thumb: img.asset ? imgUrl(img.asset, 160)  : undefined,
      alt:   img.alt ?? name,
      caption: img.caption,
    })),
    ...featureImgs.map(fi => ({
      src:   fi.asset ? imgUrl(fi.asset, 1200) : undefined,
      thumb: fi.asset ? imgUrl(fi.asset, 160)  : undefined,
      alt:   (isZh ? fi.caption?.zhCN : fi.caption?.en) ?? name,
      caption: isZh ? fi.caption?.zhCN : fi.caption?.en,
    })),
  ]
  ...
  return (
    <main>
      ...
      <div className="jm-sec-inner" style={{ paddingTop: 24, paddingBottom: 0, fontSize: 13, color: 'var(--ink-3)', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <span>/</span>
        <Link href="/products/smart-toilet" style={{ color: 'var(--ink-3)', textDecoration: 'none' }}>{t('smartToiletName')}</Link>
        <span>/</span>
        <span style={{ color: 'var(--ink)', fontWeight: 500 }}>{name}</span>
      </div>

      {/* Amazon-style 2-column grid: carousel left, info right */}
